...
void Init_sim800l() {
  ...
  answer = 0;
  while (answer == 0) {
    answer = sendAT("AT+CNMI=2,2,2,0,0", "OK",1000); // Set Message mode when receive new SMS
  }
  sprintf(aux_string,"ATD%s;",phonenumber);
  answer = 0;
  while (answer == 0) {
    answer = sendAT(aux_string, "OK", 2000); // Set Message mode when receive new SMS
  }
  delay(10000);
  answer = 0;
  while (answer == 0) {
    answer = sendAT("ATH", "OK",2000); // Set Message mode when receive new SMS
  }
}
//******************************************************************************************************
void Erase_buffer() {
